...
class downloadWaterData:
    def __init__(self, destination_folder, dataset_name):
        self.DESTINATION_FOLDER = destination_folder
        self.DATASET_NAME = dataset_name
        self.processDest()
        self.prepareLatsNLongs()
    ...
    def downloadAllData(self):
        ...
        self.downloadData(lats, longs)

    def downloadData(self, lats, longs):
        fileCount = len(longs)*len(lats)
        counter = 1
        for lng in longs:
            for lat in lats:
                print(lat, lng)
                filename = self.DATASET_NAME+ "_" + str(lng) + "_" + str(lat) + "v1_3_2020.tif"
                if os.path.exists(self.DESTINATION_FOLDER + filename):
                    print(self.DESTINATION_FOLDER + filename + " already exists - skipping")
                else:
                    url = "http://storage.googleapis.com/global-surface-water/downloads2020/" + self.DATASET_NAME + "/" + filename
